    ...
    <div className="bg-slate-800 rounded-lg overflow-hidden group transform hover:-translate-y-2 transition-transform duration-300 shadow-lg hover:shadow-cyan-500/20 flex flex-col">
      <Link to={`/events/${event.id}`} className="block">
        <div className="w-full h-56 overflow-hidden relative">
          <LazyLoadImage alt={event.title} src={`http://localhost:3000${event.imageBannerUrl}`} effect="blur" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
