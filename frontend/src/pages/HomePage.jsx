...
const HomePage = () => {
  ...
  if (loading)
    return (
      <motion.div
        className="min-h-screen flex items-center justify-center bg-gradient-to-b from-emerald-50 to-white"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      >
    ...
    <motion.div {...fadeUp(0.2)} className="text-center mt-10">
      <Link to={to}>
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          className="px-8 py-3 bg-emerald-600 text-white font-semibold rounded-full shadow-md hover:shadow-xl"
        >
          {children}
        </motion.button>
      </Link>
  ...
  return (
    <motion.div
      className="bg-gradient-to-b from-white via-emerald-50/20 to-white"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      <div className="container mx-auto px-6 lg:px-10 py-20 space-y-20">
        ...
        <motion.section
          {...fadeUp(0.1)}
          className="bg-gradient-to-br from-emerald-50 to-white p-10 rounded-3xl shadow-lg"
        >
          <SectionTitle
            title={t("homepage.our_services")}
            subtitle={t("homepage.services_subtitle")}
          />
        ...
        <motion.section
          {...fadeUp(0.1)}
          className="bg-gradient-to-br from-white to-emerald-100/60 p-10 rounded-3xl shadow-lg"
        >
            ...
            {data.events.map((e, i) => (
              <motion.div key={e.id} {...fadeUp(i * 0.1)} whileHover={{ y: -6 }}>
                <Link to={`/events/${e.id}`} className="block rounded-2xl shadow-md overflow-hidden">
                  <img
                    src={`http://localhost:3000${e.imageBannerUrl}`}
                    alt={e.title}
        ...
        <motion.section
          {...fadeUp(0.1)}
          className="bg-gradient-to-br from-emerald-50 via-white to-emerald-100 p-10 rounded-3xl shadow-lg"
        >
        ...
        <motion.section
          {...fadeUp(0.1)}
          className="bg-gradient-to-br from-white to-emerald-50 p-10 rounded-3xl shadow-lg"
        >
        ...
        <motion.section
          {...fadeUp(0.1)}
          className="bg-gradient-to-r from-emerald-50 via-white to-emerald-100 p-10 rounded-3xl shadow-xl"
        >
          ...
          >
            {data.gallery.map((g, i) => (
              <motion.div
                ...
                {...fadeUp(i * 0.05)}
                whileHover={{ scale: 1.05 }}
                className="relative group flex-shrink-0 w-[350px] h-[220px] rounded-3xl overflow-hidden shadow-lg"
              >
                <img
                  src={`http://localhost:3000${g.imageUrl}`}
                  alt={g.title}
