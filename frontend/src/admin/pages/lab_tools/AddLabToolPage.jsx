...
import { FiInfo, FiDollarSign, FiFileText, FiLink, FiImage, FiSave } from "react-icons/fi";
...
const AddLabToolPage = () => {
  ...
  const { id } = useParams();
  const [formData, setFormData] = useState({ name: "", price: "", description: "", linkTokopedia: "", linkShopee: "" });
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white/10 p-8 rounded-lg shadow-lg">
            <h2 className="text-2xl font-semibold mb-6 border-b border-slate-700 pb-4">Tool Details</h2>
            <InputField label="Tool Name" name="name" value={formData.name} onChange={handleChange} required={true} placeholder="Contoh: 'Gelas Ukur 100ml'" icon={<FiInfo />} />
            <InputField label="Price" name="price" type="number" value={formData.price} onChange={handleChange} required={true} placeholder="Contoh: 75000" icon={<FiDollarSign />} />
            <InputField label="Description" name="description" type="textarea" value={formData.description} onChange={handleChange} required={true} placeholder="Deskripsi singkat alat..." icon={<FiFileText />} />
          </div>

          <div className="bg-white/10 p-8 rounded-lg shadow-lg">
            <h2 className="text-2xl font-semibold mb-6 border-b border-slate-700 pb-4">Media & Links</h2>
            <div className="mb-6">
