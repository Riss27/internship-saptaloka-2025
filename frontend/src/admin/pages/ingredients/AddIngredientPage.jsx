...
const AddIngredientPage = () => {
  ...
  useEffect(() => {
    if (isEditMode) {
      axios
        ...
        .catch((error) => console.error("Gagal mengambil detail bahan:", error));
    }
  }, [id, isEditMode]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        await axios.put(`http://localhost:3000/api/ingredients/${id}`, submissionData, config);
      } else {
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white/10 p-8 rounded-lg shadow-lg">
            <h2 className="text-2xl font-semibold mb-6 border-b border-slate-700 pb-4">Ingredient Details</h2>
            <InputField label="Ingredient Name" name="name" value={formData.name} onChange={handleChange} required={true} placeholder="Contoh: 'Rose Oil'" icon={<FiInfo />} />
            <InputField label="Price" name="price" type="number" value={formData.price} onChange={handleChange} required={true} placeholder="Contoh: 50000" icon={<FiDollarSign />} />
            <InputField label="Description" name="description" type="textarea" value={formData.description} onChange={handleChange} required={true} placeholder="Deskripsi singkat bahan..." icon={<FiFileText />} />
          </div>
          ...
          <div className="bg-white/10 p-8 rounded-lg shadow-lg">
            ...
            <div className="mb-6">
              <label className="block mb-2 font-medium text-slate-300">Ingredient Image</label>
        ...
        <div className="mt-8">
          <button type="submit" disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-800 py-3 rounded-md font-bold text-lg flex items-center justify-center">
            ...
            {isLoading ? "Saving..." : isEditMode ? "SAVE CHANGES" : "ADD INGREDIENT"}
          </button>
        </div>
      </form>
    </div>
